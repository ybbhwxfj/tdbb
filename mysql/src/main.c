  ...
  */
  ...
  if (fd == -1) {
    ...
    if (fd == -1) {
      ...
      size_t unused = read(fd, &seed, sizeof(seed));
      close(fd);
    }
  } else {
    size_t unused = read(fd, &seed, sizeof(seed));
...
int thread_main(thread_arg *arg) {
  ...
  if (py > 0) {
    ...
    if (mysql_stmt_prepare(stmt[t_num][10],
                           ...
                           91))
      goto sqlerr;
    if (mysql_stmt_prepare(stmt[t_num][11],
    ...
    if (mysql_stmt_prepare(stmt[t_num][13],
                           ...
                           79))
      goto sqlerr;
    if (mysql_stmt_prepare(stmt[t_num][14],
    ...
    if (mysql_stmt_prepare(stmt[t_num][16],
                           ...
                           72))
      goto sqlerr;
    if (mysql_stmt_prepare(stmt[t_num][17],
                           "UPDATE customer SET c_balance = ?, c_data = ? WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?",
                           90))
      goto sqlerr;
    if (mysql_stmt_prepare(stmt[t_num][18],
  ...
  if (os > 0) {
    if (mysql_stmt_prepare(stmt[t_num][20],
                           "SELECT count(c_id) FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_last = ?",
                           79))
      goto sqlerr;
    if (mysql_stmt_prepare(
