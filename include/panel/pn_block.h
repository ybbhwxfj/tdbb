...
private:
  template<typename T>
  result<void> pnb_handle_message(const ptr<connection> &, message_type, const T &) {
    BOOST_ASSERT(false);
    return outcome::success();
  }

  result<void> pnb_handle_message(const ptr<connection> &, message_type, const panel_report &p);
  result<void> pnb_handle_message(const ptr<connection> &, message_type, const panel_info_request &p);
