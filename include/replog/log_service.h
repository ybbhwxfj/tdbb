...
public:
  ...
  static tx_log_index state_index() { return tx_log_index(0, 0, 0); }
...
class log_service {
public:
  virtual void write_log(const std::vector<ptr<raft_log_entry>> &,
                         const log_write_option &) = 0;

  virtual void write_state(const ptr<raft_log_state> &,
                           const log_write_option &) = 0;
