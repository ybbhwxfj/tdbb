 ...
 public:
  ...
  void add(message_type type,
    ...
    if (iter != total_message_.end()) {
      ...
      if (iter->second.count % 100000 == 99) {
        BOOST_LOG_TRIVIAL(info) << name_ << " send-receive message time " << enum2str(iter->first)
                                << " " << iter->second.duration2.total_milliseconds() / iter->second.count << " ms";
      }
    } else {
      total_time tt;
  ...
  void print() {
    for (auto t : total_message_) {
      if (t.second.count > 0) {
        BOOST_LOG_TRIVIAL(info) << name_ << " send-receive time " << enum2str(t.first)
                                << " " << t.second.duration2.total_milliseconds() / t.second.count << " ms";
      }
    }
    if (dispatch_time_.count > 0) {
      BOOST_LOG_TRIVIAL(info) << name_ << " dispatch time :" <<
                              dispatch_time_.duration2.total_milliseconds() / dispatch_time_.count << " ms";
    }
  }
};
