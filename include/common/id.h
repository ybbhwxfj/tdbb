...
typedef std::string shard_name_t;
...
typedef uint64_t log_index_t; // started_ from 1

//#define MAKE_NODE_ID(_az_id, _rg_id) (((_az_id) << 16) | (shard_ids))

#define __CCB_FLAG 1
#define __RLB_FLAG 2
#define __DSB_FLAG 4

inline node_id_t MAKE_NODE_ID(az_id_t az, shard_id_t sd_id,
  ...
  return sd_id << 16 | az << 8 | ccb_flag | rlb_flag | dsb_flag;
}

inline az_id_t TO_RG_ID(node_id_t node_id) { return az_id_t(node_id >> 16); }
