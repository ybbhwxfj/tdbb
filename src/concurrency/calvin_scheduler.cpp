...
void calvin_scheduler::schedule(const ptr<calvin_epoch_ops> &e) {
  LOG(trace) << id_2_name(conf_.node_id()) << " schedule epoch " << e->epoch_
             << " ";
#ifdef TX_TRACE
  ...
  // LOG(trace) << id_2_name(conf_.node_id()) << " handle epoch " << epoch << "
  // ops: " << e->ops_.size();
  if (e->reqs_.empty()) {
    send_ack(epoch, e->node_ids_);
    return;
  }
  std::vector<tx_log_binary> entry;
  ...
  for (size_t i = 0; i < e->reqs_.size(); i++) {
    ...
    ops += num_write_ops;
    ops += num_read_op;
    proto.set_xid(req->xid());
    proto.set_log_type(TX_CMD_RM_COMMIT);
    LOG(trace) << id_2_name(node_id_) << " calvin scheduler append log, " << req->xid();
    tx_log_binary log_binary = tx_log_proto_to_binary(proto);
    xlog->swap(log_binary);
    create_or_find(ctx_set, req, read_only);
  }
  // LOG(trace) << id_2_name(conf_.node_id()) << " write operations " <<
  // entry.operation().size();
  wal_->async_append(entry);
  /*
  for (auto & c : ctx_set) {
    LOG(info) << "calvin async append log " << c.second->xid();
  ...
  for (const ptr<tx_request> &req : e->reqs_) {
    ...
    for (const tx_operation &op : req->operations()) {
      lock_mode lt;
      if (op.op_type() == TX_OP_DELETE || op.op_type() == TX_OP_INSERT ||
        ...
        (*num_op)--;
        assert(false);
        LOG(error) << " unknown tx_rm op types";
        continue;
      }
      ...
      access_mgr_->lock_row(op.xid(), op.operation_id(), lt,
                            op.tuple_row().table_id(),
                            op.tuple_row().shard_id(),
                            predicate(op.tuple_row().tuple_id()), tx);
    }
  }
}

void calvin_scheduler::tx_op_done(const ptr<calvin_context> &tx,
                                  const tx_operation &op) {
  tuple_pb tuple;
...
void calvin_scheduler::send_ack(uint64_t epoch,
                                const std::set<node_id_t> &ids) {
#ifdef TX_TRACE
  ...
  for (auto id : ids) {
    ...
    msg->set_dest(id);
    msg->set_epoch(epoch);
    auto r = service_->async_send(msg->dest(), CALVIN_EPOCH_ACK, msg, true);
    if (!r) {
      LOG(error) << "async send calvin epoch ack error";
    }
  }
}
...
ptr<calvin_context> calvin_scheduler::create_or_find(
    std::unordered_map<xid_t, ptr<calvin_context>> &ctx_set,
    const ptr<tx_request> &o, bool read_only) {
