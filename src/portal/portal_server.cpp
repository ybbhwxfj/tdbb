...
int portal(int argc, const char *argv[]) {
  ...
  desc.add_options()
      ...
      ("loose-bind", boost::program_options::value<std::string>(), "true for loose bind")
  ...
  LOG(info) << "run block-db " << enum2str(block_db_type());
...
void block_run(const config &conf, const callback &fn) {
  ...
  std::string name =
      "BE_" + n + std::to_string(conf.shard_ids()[0]) + std::to_string(conf.az_id());
  ...
  for (const block_type_t &t : conf.this_node_config().block_type_list()) {
    if (t == BLOCK_CCB) {
      ...
      ptr<cc_block> ccb = std::make_shared<cc_block>(
          conf, service.get(), fn.schedule_before_, fn.schedule_after_);
      ptr<processor_sink<ptr<cc_block>>> s1(
          new processor_sink<ptr<cc_block>>(ccb, MESSAGE_BLOCK_CCB));
      blocks.push_back(ccb);
      processors[s1->message_block_type()] = s1;
    } else if (t == BLOCK_DSB) {
      ptr<ds_block> dsb = std::make_shared<ds_block>(conf, service.get());
      ptr<processor_sink<ptr<ds_block>>> s_dsb(
          new processor_sink<ptr<ds_block>>(dsb, MESSAGE_BLOCK_DSB));
      processors[s_dsb->message_block_type()] = s_dsb;
      blocks.push_back(dsb);
    } else if (t == BLOCK_RLB) {
      ptr<rl_block> rlb =
          std::make_shared<rl_block>(conf, service, fn.become_leader_,
                                     fn.become_follower_, fn.commit_entries_);
      ptr<processor_sink<ptr<rl_block>>> s_dsb(
          new processor_sink<ptr<rl_block>>(rlb, MESSAGE_BLOCK_RLB));
      processors[s_dsb->message_block_type()] = s_dsb;
  ...
  message_handler handler = [s, processors, srv_ptr,
    ...
    try {
      ...
      if (id == CLOSE_REQ) {
        srv_ptr->stop();
        LOG(info) << id_2_name(conf.node_id()) << " receive close";
      } else {
        if (p) {
      ...
      if (ms > TEST_HANDLE_MAX_MS) {
        LOG(info) << " process message " << ms << "ms, " << enum2holder(id);
      }
      if (print_message) {
        // LOG(info) << " process message " << enum2holder(id);
      }
#endif // #ifdef TEST_HANDLE_TIME
  ...
  proto_message_handler local_handler =
      [s, processors, srv_ptr,
          &conf](ptr<connection> conn, message_type id,
                 ptr<google::protobuf::Message> msg) -> result<void> {
        try {
#ifdef TEST_HANDLE_TIME
          auto b = std::chrono::steady_clock::now();
#endif // TEST_HANDLE_TIME
  ...
  ptr<debug_server> debug(new debug_server(
      "0.0.0.0", uint16_t(conf.this_node_config().port() + 1000),
      debug_handler));
  ...
  server->join();
  LOG(info) << "block-db " << id_2_name(conf.node_id()) << " stopped ";
  debug->stop();
  debug->join();
  sleep(3);
}